...
Implemented by `supabase/migrations/20260922210100_privacy_foundation.sql` and `20260922210200_privacy_tracking_foundation.sql`, with the controlled request/subject procedures added by `20260923140000_privacy_operations_core.sql` — still no disclosure execution, ledger service, fulfillment or runtime read paths. These supporting catalog tables make notice/purpose versioning explicit rather than putting unreviewed strings on candidate rows.
...
Tenant. Columns: `request_id uuid → privacy_requests`, `candidate_id uuid?`, `legacy_record_id uuid? → legacy_records`, `candidate_version bigint? CHECK (>0)`, `legacy_version bigint? CHECK (>0)`, `source_sha256 bytea?`, `reviewed_by_membership_id uuid → organization_memberships`, `reviewed_at timestamptz`. Exactly one of `candidate_id`/`legacy_record_id` must be set; candidate targets require `candidate_version`, legacy targets require `source_sha256` and forbid `candidate_version`. `legacy_version` was added by the privacy operations core; historical subjects may carry NULL, but the implemented procedures re-review and populate the matching snapshot version before acting on a legacy target. `legacy_version` additionally requires `legacy_record_id` (CHECK `legacy_version IS NULL OR legacy_record_id IS NOT NULL`), so candidate subjects cannot carry an irrelevant legacy version.
...
Tenant. Columns: `dataset_id uuid → legacy_datasets`, `native_id bigint`, `candidate_id uuid? → candidates`, `source_sha256 bytea?`, `verified_by_membership_id uuid? → organization_memberships`, `verified_at timestamptz?`, `processing_restricted boolean DEFAULT false`, `lifecycle_generation bigint DEFAULT 1 CHECK (>0)`, `updated_at timestamptz`, `version bigint`. Unique `(organization_id,dataset_id,native_id)`; same-organization composite FKs for dataset, candidate and reviewer. `native_id` accepts any bigint: legacy identity is preserved, not validated. `processing_restricted` and `lifecycle_generation` were added by the privacy operations core for local restriction enforcement on verified records; neither changes the source public-data payload or fabricates a canonical candidate.
...
Index `(organization_id,occurred_at DESC,id DESC)` and `(organization_id,target_type,target_id,occurred_at,id)`, plus actor-user and `(organization_id,actor_membership_id,actor_user_id)` covering indexes. Target ID is intentionally non-FK so evidence can outlive erased objects. Staff rows require actor user and membership belonging to that user: a CHECK enforces presence for staff-kind rows and the composite foreign keys prove the membership belongs to that user and tenant. `details` uses per-action allowlists and max 4 KiB; before/after role grants can contain permission keys, never CV/text/token/PII dumps. The privacy operations core replaced the two-action CHECK with a named `audit_events_action_check` retaining the staff branches unchanged and adding `actor_kind='staff'`, `target_type='privacy_request'` actions `privacy.request.created`, `privacy.request.verified`, `privacy.subject.reviewed`, `privacy.candidate.corrected`, `privacy.subject.restricted` with `details` keys limited to `subject_id`, `target_id`, `target_kind`, `previous_version`, `new_version`, `target_previous_version`, `target_new_version`, `changed_fields`, `lifecycle_generation`, `enforcement_scope` — identifiers, versions and scope labels only, never personal values or fingerprints. Future actions still require a reviewed constraint extension. Forced RLS; insert-only through the executor policy, no runtime read interface yet; operator-controlled expiry/redaction.
