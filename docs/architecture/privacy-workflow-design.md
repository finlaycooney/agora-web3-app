...
- Follow the approved organization -> candidate -> application/document/blob -> queue/event order. Resolve case/target locks consistently within that order and document it before code; do not lock a queue then wait on its candidate.
- Recheck authority after acquiring governing locks. Require expected versions, explicit case scope and application-generated audit/correlation IDs. Failure must roll back the state change, event and queued effects together.
...
Approval record: `[APPROVE: technical maintainer, privacy owner, legal reviewer, exact scope, amended column contracts, effective decisions and unresolved activation gates]`. Workflow execution remains pending; the data foundations are implemented locally.
