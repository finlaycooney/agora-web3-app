...
| `create_privacy_request_v1(request_id, kind, received_at, due_at, audit_id, correlation_id)` — implemented | `privacy.manage`; registers a `received` request in context organization with no guessed candidate mapping; finite `received_at` not in the future, optional `due_at` ≥ `received_at`; event + audit atomically |
| `review_privacy_subject_v1(request_id, expected_request_version, subject_id, candidate_id, legacy_record_id, expected_target_version, source_sha256, audit_id, correlation_id)` — implemented | Same; request locked with expected version; exactly one target; candidate targets lock the scoped candidate (active/restricted only) and forbid a fingerprint; legacy targets lock the scoped record in an active dataset and require stored verification plus the reviewed 32-byte fingerprint; an existing subject cannot be retargeted across case or target; snapshots `candidate_version`/`legacy_version`; request version advances once |
| `verify_privacy_request_v1(request_id, expected_version, verification_method, audit_id, correlation_id)` — implemented | Same; `received` requests only; lowercase machine-code method; at least one reviewed subject; stamps verifier membership/time/method; operator assertion, not authentication; no identification documents |
| `correct_privacy_candidate_v1(request_id, expected_request_version, subject_id, expected_candidate_version, changes, audit_id, correlation_id)` — implemented | Same; `correction` kind in `verified`/`in_progress`; changes object limited to `full_name` (null or nonblank, ≤256 chars) / `professional_summary` (≤10000 chars), values string-or-null (a null clears the stored value), payload ≤49152 bytes, at least one real change; reviewed subject's candidate locked and both caller and reviewed versions must match; bumps profile/entity versions, records changed field names only; scope `candidate_profile_only` — legacy/application copies and recipient notification untouched |
| `restrict_privacy_subject_v1(request_id, expected_request_version, subject_id, expected_target_version, audit_id, correlation_id)` — implemented | Same; `restriction`/`erasure`/`correction`/`objection`/`withdrawal` kinds in `verified`/`in_progress`; reviewed target locked, caller and reviewed versions plus legacy fingerprint must match; candidate restriction increments lifecycle generation, restricts active documents/purposes, bumps live/unavailable/retired blob generations and cancels `intended` disclosures (`candidate_records_only`); legacy restriction sets `processing_restricted` and cancels that record's `intended` disclosures (`legacy_metadata_only`); refuses already-restricted targets and scopes beyond 1000 affected rows (54000); request stays `in_progress`, ledger fields untouched |
| `save_client_v1(client_id, expected_version, fields, operation_id, correlation_id)` — implemented | `clients.read` + `clients.write`; `expected_version NULL` creates only (existing id → 23505, never upsert); all `ClientInput` fields validated in SQL (`client_fields_valid_v1`); `version` advances on every save, `public_profile_version` only when `name`/`is_stealth`/`anonymous_description` change; stealth → named reveal additionally requires an Admin membership (42501 otherwise); receipt + audit atomically |
| `create_job_draft_v1(job_id, revision_id, client_id, fields, operation_id, correlation_id)` — implemented | `jobs.read` + `jobs.write` + `clients.read`; client must be active and configured (`client_configured_v1`); inserts on the active `default` pipeline (23514 when absent); creates the job row and revision 1 in one draft; incomplete drafts (title + client only) are valid; never auto-publishes |
| `save_job_draft_v1(revision_id, expected_version, fields, operation_id, correlation_id)` — implemented | Same grants; locks organization → client → job → revision in order; `draft` status only (23514); revision version conflict 40001; updates draft content/text/version only — the job row and published revision are untouched |
| `begin_job_revision_v1(job_id, revision_id, expected_job_version, operation_id, correlation_id)` — implemented | Same grants; requires a `published_revision_id` (23514 otherwise — legacy jobs need a reviewed backfill); an existing draft raises 23505 (fetch it, no parallel drafts); copies the published content into a new draft revision with NULL publication metadata; job row and pointer unchanged |
| `duplicate_job_v1(source_revision_id, expected_source_version, client_id, job_id, revision_id, operation_id, correlation_id)` — implemented | Same grants; copies any reviewed revision's content into a new job + draft revision 1 under a validated configured client (same or different); applications, publication metadata, receipts and audit are never copied; new job id/slug |
| `preview_job_public_v1(revision_id)` — implemented | `jobs.read` + `clients.read`; validates the draft against the ready rules, computes `job_public_projection_v1` (title/employment/mode/locations/regions/compensation/bonuses/descriptionDocument/company label only — no internal ids, contacts or real stealth name) and returns `{projection, revisionVersion, clientVersion, reviewHash}` where the hash binds the exact reviewed bundle |
| `publish_job_revision_v1(revision_id, expected_version, expected_client_version, review_hash, operation_id, correlation_id)` — implemented | Same write grants; version mismatches and hash mismatches raise 40001; supersedes the previous published revision (status flip only), stamps publication metadata + client profile snapshot, updates the job row's derived display fields and `published_revision_id`, and bumps both versions once; the only path to `publication_state = 'published'`; `application_state` untouched |
| `get_client_v1(client_id)` — implemented | `clients.read`; returns the internal client DTO including contact fields |
| `get_job_workspace_v1(job_id)` — implemented | `jobs.read` + `clients.read`; returns the job, the draft and published revision DTOs, and `publicationNeedsReview` when the client's public profile advanced past the stored snapshot |
| `get_job_publication_v1(job_id)` — implemented | Same; returns the stored safe projection or NULL when unpublished, the client is inactive/unconfigured, or `public_profile_version` no longer matches — publication fails closed after name/stealth/brief changes until a new preview+publish |
...
| `save_job_v1(job_id, expected_version, allowed_fields)` | Superseded by the implemented `save_job_draft_v1`/`begin_job_revision_v1` draft-revision contract above |
| `publish_job_v1(job_id, expected_version, reviewed_content_hash)` | Superseded by the implemented `preview_job_public_v1`/`publish_job_revision_v1` contract above |
...
The mutation interfaces take caller-generated `audit_id`/`correlation_id` UUIDs: the application mints UUIDv4 values per request, the database never generates UUIDs, and a duplicate `audit_id` rolls back the whole mutation so retries cannot fabricate divergent evidence. Version parameters accept decimal strings across the full PostgreSQL `bigint` range (the JavaScript seam passes them through unchanged; unsafe numbers are rejected) so optimistic-concurrency values never lose precision.

The five implemented privacy procedures are low-volume case operations with a fixed lock order: organization `FOR UPDATE`, then the privacy request `FOR UPDATE`, then the reviewed candidate/legacy target `FOR UPDATE`, then affected blobs, documents, purposes and disclosures each in UUID order, then the privacy event and audit row. Child-row locks use the exact affected-row predicates (live/unavailable/retired blobs, active documents and purposes, `intended` disclosures), so terminal child rows such as sent disclosures are never locked. `privacy.manage` is re-checked in a new statement after the organization lock so a revoked grant fails the waiter; any isolation level other than read committed is rejected (SQLSTATE 25001) so a stale snapshot cannot hide revocation. Every procedure emits one `privacy_events` row and one `audit_events` row sharing the caller-generated `audit_id`; audit `details` is limited to the new check's key allowlist (subject/target IDs, kinds, versions, generation, scope, changed field names) — no personal values, fingerprints or document text. The procedures cannot fulfill or reject requests, confirm ledger entries, delete bytes, lift restrictions or retarget reviewed evidence; fulfillment, disclosure execution and ledger delivery remain separate unimplemented workflows.

The ten client/job procedures share the authorization skeleton from the privacy batch — `recruitment_actor_v1` asserts context, isolation (25001 on anything but read committed), the organization row lock and the full permission conjunction in a fresh statement after the lock. Read paths take `FOR SHARE`; writes take `FOR UPDATE` then lock client → job → revision in that order. Every write takes a stable caller operation UUID plus a server correlation UUID: `recruitment_receipt_v1` looks up the `(organization_id, operation_id)` receipt after the permission recheck, replays return the stored result with `replayed: true`, and a kind/actor/digest mismatch raises 23505; `recruitment_record_v1` then inserts the audit event and receipt in the same statement. Receipt digests hash the canonical `jsonb` of kind + every semantic argument (never operation/correlation ids), so an identical retry returns the original ids and versions while a changed payload, actor or tenant can never replay the old result. `publish_job_revision_v1` recomputes the exact preview bundle hash, so a stale or edited draft fails 40001 rather than publishing unreviewed content; it also re-checks `expected_client_version` so a concurrent public-profile change fails closed. The public projection helper emits only the reviewed shape — title, employment/mode, structured locations and remote regions, compensation, bonuses, description document and the company label (`'Stealth company'` + anonymous brief when stealth, otherwise the client name) — and applies deterministic literal guardrails that reject stealth projections whose draft title, description text or company brief still contain the real client name, contact email, contact name, website host or social URLs. Those checks are guardrails, not universal de-identification; a human preview review remains required. `get_job_publication_v1` serves only the stored snapshot and returns NULL once `public_profile_version` no longer matches, so public-profile edits invalidate publication while private contact edits do not.
